...
class ImportFile(BaseImport):
    def __init__(self, env, import_record_id):
        ...
        self.uom_obj = self.env['product.uom']
    ...
    def get_category(self, categories_list, parent=False, public=False):
        if categories_list:
            if DEBUG:
            ...
            if len(categories) == 1:
                ...
                if categories_list:
                    return self.get_category(categories_list, parent=categories[0], public=public)
                else:
                    return categories[0]
                ...
                self.new += 1
                if categories_list:
                    return self.get_category(categories_list, category, public)
                else:
                    return category
        return False
    ...
    def import_row(self, row_list):
        ...
        if not len(row_list) == len(self.HEADER):
            ...
            error = u"""Row {row}: Row_list is {row_len} long. We expect it to be {expected} long, with this columns:
                ...
                """.format(
                    ...
                    keys=self.HEADER,
                    header=', '.join([unicode(v) for v in zip(self.HEADER, row_list)]))

            _logger.error(unicode(row_list))
            _logger.error(error)
            self.error.append(error)
            if DEBUG:
        ...
        if hasattr(record, 'uom') and record.uom:
            vals_product['uom_id'] = self.get_uom(record.uom).id
            vals_product['uom_po_id'] = vals_product['uom_id']
        ...
        if hasattr(record, 'ean13') and record.ean13:
            if check_ean(record.ean13):
                ...
                if 'ean13' in self.PRODUCT_ERRORS:
                    error = u"Row {0}: '{1}' is not a valid EAN13 code".format(self.processed_lines, vals_product['ean13'])
                    _logger.error(error)
                    self.error.append(error)
                    return False
