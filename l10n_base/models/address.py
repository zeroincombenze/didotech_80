...
class ResCity(models.Model):
    ...
    region = fields.Many2one(related='province_id.region', string=_('Region'), readonly=True)
...
class Address(models.AbstractModel):
    ...
    def check_category(self):
        countries = self.env['res.country'].search([('name', '=', self.country_id.name)])
        if countries:
            country = countries[0]
    ...
    enable_province = fields.Boolean(compute=check_category, string='Provinces?', readonly=True)
    enable_region = fields.Boolean(compute=check_category, string='Regions?', readonly=True, )
    enable_state = fields.Boolean(compute=check_category, string='States?', readonly=True, default=True)
    auto_off = fields.Boolean(compute=dummy, string=_('Auto off'), help="Unlock address fields")
    ...
    def address_autocomplete(self, city):
        ...
        self.country_id = city.region.country_id and city.region.country_id.id or False
        self.city = city.name
        self.find_city = True
    ...
    def on_change_zip(self):
        if self.zip and len(self.zip) > 3:
            ...
            if not city_ids:
                city_ids = city.search([('zip', '=ilike', self.zip[:3] + 'xx')])
    ...
    def on_change_city(self):
        if self.city:
            cities = self.env['res.city'].search([('name', '=ilike', self.city.title())])
            if cities:
                city = cities[0]
                if not self.zip:
                    self.zip = city.zip
    ...
    def on_change_province(self):
        if self.province:
            self.region = self.province.region and self.province.region.id or False

    @api.one
    @api.onchange('region')
    def on_change_region(self):
        if self.region:
            self.country_id = self.region.country_id and self.region.country_id.id or False

    def _set_vals_city_data(self, vals):
        if 'city' in vals and 'province' not in vals and 'region' not in vals:
            if vals['city']:
                cities = self.env['res.city'].search([('name', '=ilike', vals['city'].title())])
                if cities:
                    city = cities[0]
                    if 'zip' not in vals:
                        vals['zip'] = city.zip
