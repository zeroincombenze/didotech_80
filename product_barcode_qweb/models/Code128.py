...
class Code128:
    ...
    CharSetA = {
        ...
        'FNC3': 96, 'FNC2': 97, 'SHIFT': 98, 'Code C': 99, 'Code B': 100, 'FNC4': 101, 'FNC1': 102,
    ...
    CharSetC = {
        ...
        '96': 96, '97': 97, '98': 98, '99': 99, 'Code B': 100, 'Code A': 101, 'FNC1': 102,
        'START A': 103, 'START B': 104, 'START C': 105, 'STOP': 106
    }
            ...
            if len(code[c:]) >= 4 and code[c: c+4].isdigit() and current_charset != self.CharSetC or \
                ...
                if current_charset != self.CharSetC:
                    ...
                    if pos:
                        ...
                        control_sum += pos * current_charset['Code C']
                    else:
                        strCode = self.ValueEncodings[self.CharSetC['START C']]
                        control_sum = self.CharSetC['START C']
                ...
                if pos:
                    ...
                    control_sum += pos * current_charset['Code B']
                else:
                    strCode = self.ValueEncodings[self.CharSetB['START B']]
                    control_sum = self.CharSetB['START B']
                ...
                if pos:
                    ...
                    control_sum += pos * current_charset['Code A']
                else:
                    strCode += self.ValueEncodings[self.CharSetA['START A']]
                    control_sum = self.CharSetA['START A']
    ...
    def getImage(self, value, height=25, extension="PNG", path="/tmp/"):
        ...
        #
        # font_filelike = BytesIO(open(path + "courB08.pil", 'rb').read())
        # # ImageFont.load(BytesIO(decompress(decodestring(courB08_pil))))
        # image = Image.open(BytesIO(open(path + "courB08.pbm", 'rb').read()))
        # font = ImageFont.ImageFont()._load_pilfont_data(font_filelike, image)
        # or
        ...
        for bit in range(len(bits)):
            if bits[bit] == '1':
                ...
                draw.rectangle(((bit + position, 0), (bit + position, height - 1)), fill=0)
...
def decodeFontFile(data, file_path):
    ...
    if not exists(file_path):
        # Write font file
        open(file_path, "wb").write(decompress(decodestring(data)))
