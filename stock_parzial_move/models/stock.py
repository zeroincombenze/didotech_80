...
class StockMove(models.Model):
    ...
    new_picking = fields.Boolean('New DDT', help='Create new DDT (if necessary) on quantity diminishing', default=True)
    ...
    # Using old API here, because I need to be able to use warning returned by the function
    def onchange_quantity(self, cr, uid, ids, product_id, product_qty, product_uom, product_uos, new_picking=False):
        ...
        if product_uos and product_uom and (product_uom != product_uos):
            precision = self.pool['decimal.precision'].precision_get(cr, uid, 'Product UoS')
    ...
    def write(self, values):
        if 'move_lines' in values:
            for line in values['move_lines']:
                if line[0] == 1 and 'new_qty' in line[2] and line[2]['new_qty']:
                    move = self.env['stock.move'].browse(line[1])
                    if ('new_picking' in line[2] and line[2]['new_picking']) \
                            or ('new_picking' not in line[2] and move.new_picking):
